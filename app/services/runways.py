...
MANUAL_RUNWAY_SUPPLEMENTS = {
    # OurAirports data can lag behind chart/runway naming used in DHMI PDFs.
    # LTAC IAC charts include RWY 03C; expose it as a selectable chart runway
    # even if the open runway dataset does not list it.
    "LTAC": [
        {
            "ident": "03C",
            "le": "03C",
            "he": "",
            "heading": 30,
            "reciprocalHeading": 210,
            "lengthFt": None,
            "widthFt": None,
            "surface": "chart",
            "lighted": False,
            "source": "manual-chart-supplement",
        },
    ],
}
...
def load_runways():
    ...
    for icao, runways in MANUAL_RUNWAY_SUPPLEMENTS.items():
        existing = {
            value
            for runway in by_airport.get(icao, [])
            for value in (runway.get("le"), runway.get("he"), runway.get("ident"))
            if value
        }
        for runway in runways:
            if runway["ident"] not in existing and runway["le"] not in existing:
                by_airport.setdefault(icao, []).append(runway)
